  ...
  module Filter
    ...
    class Sjis < Base
      ...
      def apply_incoming?(controller)
        ...
        # 携帯電話の場合に適用する。
        # スマートフォン、Docomoの携帯もUTF-8にはしない
        mobile = controller.request.mobile
        mobile && !(mobile.instance_of?(Jpmobile::Mobile::Vodafone)||mobile.instance_of?(Jpmobile::Mobile::Softbank)||mobile.instance_of?(Jpmobile::Mobile::Docomo)||mobile.instance_of?(Jpmobile::Mobile::Smartphone))
      end
      def apply_outgoing?(controller)
        [nil, "text/html", "application/xhtml+xml"].include?(controller.response.content_type) &&
