...
(function(tinymce) {
	...
	tinymce.create('tinymce.EditorCommands', {
		...
		Indent : function() {
			var ed = this.editor, d = ed.dom, s = ed.selection, e, iv, iu, ml;
			...
			if (ed.settings.inline_styles && (!this.queryStateInsertUnorderedList() && !this.queryStateInsertOrderedList())) {
				each(s.getSelectedBlocks(), function(e) {
					...
					if (e.nodeName == 'TABLE') {
						if (e.style.marginLeft == 'auto')
							ml = parseInt(e.style.paddingLeft || 0);
						else
							ml = e.style.marginLeft;

					 	d.setStyle(e, 'marginLeft', (parseInt(ml || 0) + iv) + iu);
					}
				});
...
*/
		...
		Outdent : function() {
			var ed = this.editor, d = ed.dom, s = ed.selection, e, v, iv, iu, vt, ml;
			...
			if (ed.settings.inline_styles && (!this.queryStateInsertUnorderedList() && !this.queryStateInsertOrderedList())) {
				each(s.getSelectedBlocks(), function(e) {
					...
					if (e.nodeName == 'TABLE') {
						...
						vt = Math.max(0, parseInt(ml || 0) - iv);
						d.setStyle(e, 'marginLeft', vt ? vt + iu : '');
					}
				});
...
*/
		...
		mceJustify : function(c, v) {
			...
			if (nn == 'TABLE') {
				...
				if (rm) {
					if (v == 'center')
						dom.setAttrib(bl || n.parentNode, 'align', '');
				...
				if (v == 'center') {
					...
					dom.setAttrib(bl, 'align', v);
					dom.setStyle(n, 'float', '');
					dom.setStyle(bl, 'marginLeft', 'auto');
					dom.setStyle(bl, 'marginRight', 'auto');
				} else {
					dom.setStyle(n, 'float', v);
					dom.setAttrib(bl || n.parentNode, 'align', '');
					dom.setStyle(n, 'marginLeft', '');
					dom.setStyle(n, 'marginRight', '');
				}
			...
			if (ed.settings.inline_styles && ed.settings.forced_root_block) {
				...
				each(se.getSelectedBlocks(dom.getParent(se.getStart(), dom.isBlock), dom.getParent(se.getEnd(), dom.isBlock)), function(e) {
					if (e.nodeName == 'TABLE') {
						if (v == 'center') {
							dom.setAttrib(e, 'align', v);
							dom.setStyle(e, 'textAlign', '');
							dom.setStyle(e, 'float', '');
							dom.setStyle(e, 'marginLeft', 'auto');
							dom.setStyle(e, 'marginRight', 'auto');
						} else {
							dom.setAttrib(e, 'align', v);
							dom.setStyle(e, 'textAlign', '');
							dom.setStyle(e, 'float', '');
							dom.setStyle(e, 'marginLeft', '');
							dom.setStyle(e, 'marginRight', '');
						}
					} else {
