class System::GroupHistoryTemporary < ActiveRecord::Base
  ...
  has_many   :user_group, :foreign_key => :group_id,  :class_name => 'System::UsersGroupHistoryTemporary'
  has_and_belongs_to_many :users, :class_name => 'System::UserTemporary',
    :join_table => 'system_users_group_history_temporaries', :order => 'job_order,system_user_temporaries.name_en'

  validates_presence_of :state,:code,:name,:start_at
  validates_uniqueness_of :code, :scope => [:parent_id,:start_at]
  validates_uniqueness_of :name, :scope => [:parent_id,:start_at]
  ...
  def self.usable?(g_id,day=nil)
    ...
    g = System::GroupHistoryTemporary.find(g_id)
  ...
  def self.get_gid(u_id = Site.user.id , day=nil)
    if day==nil
      ...
      ug_cond   = "user_id = #{u_id} and job_order=1"
      user_group = System::UsersGroupHistoryTemporary.find(:all , :conditions=> ug_cond ,:order => ug_order)
      group = System::GroupHistoryTemporary.find(user_group[0].group_id) unless user_group.blank?
      return group.id
    else
      ug_order  = "user_id ASC , start_at DESC , job_order ASC"
      ug_cond   = "user_id = #{u_id} and job_order=1 and start_at < #{day}"
      user_group = System::UsersGroupHistoryTemporary.find(:all , :conditions=> ug_cond ,:order => ug_order)
      return nil if user_group.blank?
      user_group.each do |g|
        return g.id if System::GroupHistoryTemporary.usable?(g.id , nil)==true
      end
      return nil
    end
  end
  ...
  def self.get_groups(user = Site.user)
    ...
    u_groups = System::UsersGroupHistoryTemporary.find(:all,:conditions=>g_cond,:order=>g_order)
    return nil if u_groups.blank?
    groups = []
    u_groups.each do |ug|
      groups << System::GroupHistoryTemporary.find(ug.group_id)
    end
    return groups
  end
  ...
  def self.select_dd_group(day=nil,level=nil,parent_id=nil,all=nil)
    ...
    if parent_id ==nil
      if level==nil
        dd_lists = System::GroupHistoryTemporary.select_dd_tree(all)
      else
      end
    else
      g_order="code ASC , start_at DESC"
      g_cond="parent_id='#{parent_id}' and state = 'enabled'"
      groups = System::GroupHistoryTemporary.find(:all,:conditions=>g_cond,:orde=>g_order)
      groups.each do |g|
        next if System::GroupHistoryTemporary.usable?(g.id , "#{day}" )==false
  ...
  def self.select_dd_tree(all=nil)
    dd_lists = []
    dd_lists << ['すべて',0] if all == 'all'
    roots = System::GroupHistoryTemporary.find(:all,:conditions=>"level_no=1 and state='enabled'")
    roots.each do |r|
      dd_lists << ['('+r.code+')'+r.name,r.id]
      dd_lists = System::GroupHistoryTemporary.get_childs(dd_lists,r)
    end
    return dd_lists
  end
  ...
  def self.get_childs(dd_lists,parent)
    c_lists = dd_lists
    childs = System::GroupHistoryTemporary.find(:all,:conditions=>"state='enabled' and parent_id=#{parent.id}")
    return c_lists if childs.blank?
    pad_str = "　"*(parent.level_no.to_i-1)*2+"+"+"-"*(parent.level_no.to_i)*2
    childs.each do |c|
      c_lists << [pad_str+'('+c.code+')'+c.name,c.id]
      c_lists = System::GroupHistoryTemporary.get_childs(c_lists,c)
    end
    return c_lists
  end
  ...
  def self.get_group_tree(_group_id)
    ...
    if _group_id.blank?
        ...
        _dep_order = "code ASC"
        _departments = System::GroupHistoryTemporary.find(:all , :conditions => _dept_conditions ,:order => _dep_order )
        ...
        _groups << _dep
            ...
            _sec_order = "code ASC"
            _sections = System::GroupHistoryTemporary.find(:all , :conditions => _sec_conditions ,:order => _sec_order )
  ...
  def self.create_table(table_name=nil,org_table='system_group_histories')
    return if table_name==nil
    connect = self.connection()
  ...
  def self.insert_table(table_name=nil,org_table='system_group_histories')
    return if table_name==nil
    connect = self.connection()
  ...
  def self.children(item)
    ...
    c_cond  = "parent_id=#{item.id} and state='enabled' and ( end_at is null or end_at >= '#{start_date}')"
    c_order = "code"
    groups = System::GroupHistoryTemporary.find(:all , :conditions=>c_cond , :order=>c_order)
    return groups
  end
