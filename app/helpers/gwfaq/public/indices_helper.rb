module Gwfaq::Public::IndicesHelper
  ...
  def gwfaq_form_indices_date
    ...
    for item in @items
      ...
      l_line = l_line + 1
      ret += tr.to_s
      ret += '<td class="docTitle ">' + link_to(hbr(item.title), item.show_path + "&pp=#{l_line}" + gwfaq_params_set) +  "</td>"
  ...
  def gwfaq_admin_form_indices_date
    ...
    for item in @items
      l_line = l_line + 1
      ret += "<tr>\n"
      ret += '<td style="text-align: center;">' + link_to(item.ststus_name, item.show_path + "&pp=#{l_line}" + gwbbs_params_set) + "</td>\n"
