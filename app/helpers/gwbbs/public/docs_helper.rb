module Gwbbs::Public::DocsHelper

  def is_comment_delete(section_code, editor_id, is_admin)
    ret = false
    ret = true if is_admin
    ret = true if section_code == Site.user_group.code
    ret = true if editor_id == Site.user.code
    return ret
  end

  def section_bbs_create_link
    title = Gwbbs::Control.find_by_create_section(Site.user_group.code)
    str = ''
    if title.blank?
