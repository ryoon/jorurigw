module Digitallibrary::Public::IndicesHelper
  ...
  def digitallib_form_indices_group
    ...
    for item in @items
      ...
      ret += '<td ' + retstr_attache(item.attachmentfile) + 'style="text-align: center;">' + "</td>\n"
      ret += '<td ' + retstr_balloon(item.one_line_note)  + 'style="text-align: center;">' + "</td>\n"
      ret += '<td style="text-align: left;">' + link_to(hbr(item.title), item.show_path(params[:state]) + "&pp=#{l_line}&grp=#{h params[:grp]}" + digitallib_params_set) + "</td>\n"
