module Gwqa::Public::DocsHelper
  ...
  def anser_sort_link(item)
    if params[:qsort].blank?
      link_to('回答の並び順<span>▲</span>', "#{item.show_path}#{gwqa_params_set}&qsort=DESC",:title=>"現在昇順、クリックで降順に並び替え")
    else
      link_to('回答の並び順<span>▼</span>', "#{item.show_path}#{gwqa_params_set}",:title=>"現在降順、クリックで昇順に並び替え")
    end
  end
