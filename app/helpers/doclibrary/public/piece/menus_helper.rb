module Doclibrary::Public::Piece::MenusHelper
  ...
  def doclibrary_folder_li(item,last)
    ...
    if item.state == 'public'
      level_no = 'folder'
      level_no = 'f_plus' unless item.children.count == 0
      ...
      if params[:f] == 'op'
        unless item.id.to_s == params[:cat].to_s
          ...
          level_no = 'f_plus current' unless item.children.count == 0
        end
      else
        level_no = 'open' if @folders[item.level_no].to_s == item.id.to_s
        level_no = 'open current' if item.id.to_s == params[:cat].to_s
      ...
      if level_no == 'open current'
        level_no = level_no + " noneFolder" if item.children.count == 0
      end
      if last.to_i == 1
      ...
      ret = "<li class='#{level_no}'><a href='#{@title.item_home_path}docs?title_id=#{item.title_id}&cat=#{item.id}#{strparam}'>#{h(item.name)}</a></li>"
    end
    return ret
  end
end
