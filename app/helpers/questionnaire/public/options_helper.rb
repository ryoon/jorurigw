...
module Questionnaire::Public::OptionsHelper

  def option_form(form, item)
    #return '' unless item.class.include?(System::Model::Unid::Recognition)
    return render :partial => 'questionnaire/public/menus/form_fields/options/form', :locals => {:f => form, :item => item}
  end

  def option_view(item)
    #return '' unless item.class.include?(System::Model::Unid::Recognition)
    return render :partial => 'questionnaire/public/menus/form_fields/options/view', :locals => {:item => item}
  end
  ...
  def option_list_array(id)
    ...
    return JsonParser.new.parse(item.option_body)
  end
  ...
  def answer_group_field_table(field_id)
    ...
    groups = []
    groups = JsonParser.new.parse(item.group_body) unless item.group_body.blank?
    table_th = ''
    for group in groups
      table_th = %Q[#{table_th}<th>#{group['field_label']}</th>]
    end
    table_th = "<tr>#{table_th}</tr>" unless table_th.blank?
    return {:groups=>groups,:table_th=>table_th}
  end
