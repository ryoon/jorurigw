class Gw::Public::EditTabsController < ApplicationController
  ...
  def create
    ...
    return authentication_error(403) unless @u_role==true
    @item = Gw::EditTab.new
    redirect_url = "#{Site.current_node.public_uri}#{@item.id}?pid=#{@parent.id}"
  ...
  def destroy
    ...
    item.save(false)
    flash[:notice] = "既に実績は作成されています。"
    location = "#{Gw.chop_with(Site.current_node.public_uri,'/')}?pid=#{@parent.id}"
