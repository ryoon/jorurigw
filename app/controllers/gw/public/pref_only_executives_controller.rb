class Gw::Public::PrefOnlyExecutivesController < ApplicationController
  ...
  def state_change
    ...
    @item.save(false)
    @set = Gw::PrefDirector.find_by_uid(params[:id])
    unless @set.blank?
      @set.state = new_state
      @set.save(false)
    end
    return redirect_to(Gw.chop_with(Site.current_node.public_uri,'/'))
  end
