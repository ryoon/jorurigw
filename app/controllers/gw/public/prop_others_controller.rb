class Gw::Public::PropOthersController < Gw::Public::PropGenreCommonController
  ...
  def index
    ...
    @page = _items.paginate(:page => params[:page], :per_page => params[:limit], :page_count=>_items.length) # ページャー用
    @items = _items[30 * (nz(params[:page], 1).to_i - 1)..30 * (nz(params[:page], 1).to_i) - 1]
  end
