class Gw::Public::MemosController < ApplicationController
  ...
  def create
    ...
    if @item.save_with_rels params[:item], :create
      ...
      redirect_to '/gw/memos?s_send_cls=2'
    else
  ...
  def list
    ...
    if !params[:finished_submit].blank?
      ...
      redirect_uri = "/gw/memos?s_send_cls=#{s_send_cls}&s_index_cls=#{s_index_cls}&s_finished=#{s_finished}"
