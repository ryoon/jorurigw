class Gw::Public::PrefExecutiveAdminsController < ApplicationController
  ...
  def csvput
    ...
    case par_item[:csv]
    when 'put'
      filename = "県幹部在庁表示管理_#{par_item[:nkf]}_#{Time.now.strftime('%Y%m%d_%H%M')}.csv"
  ...
  def csv_output(items)
    ret = %Q("県幹部表示順","Gw及び議会表示","県三役表示","役職","職員番号","氏名"\n)
    items.each do |u|
      ret += %Q("#{u.u_order}",)
