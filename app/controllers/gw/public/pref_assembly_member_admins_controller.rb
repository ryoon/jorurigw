class Gw::Public::PrefAssemblyMemberAdminsController < ApplicationController
  ...
  def csvput
    ...
    case par_item[:csv]
    when 'put'
      filename = "gw_pref_assembly_members_#{par_item[:nkf]}.csv"
  ...
  def csv_output(items)
    ...
    items.each do |u|
      ret += "#{u.g_order},#{u.g_name},#{u.u_order},#{u.u_lname},#{u.u_name}"
      ret += ",退席"if u.state == "off"
      ret += ",在席"if u.state == "on"
      ret += "\n"
    end
    return ret
  end
    ...
    when 'up'
      if par_item.nil? || par_item[:nkf].nil? || par_item[:file].nil?
        ...
        if file.blank?
          ...
          Gw::PrefAssemblyMember.update_all("state='off'" ,"state='退席'")
          Gw::PrefAssemblyMember.update_all("state='onn'" ,"state='在席'")
        end
        location = Gw.chop_with("#{Site.current_node.public_uri}",'/')
        redirect_to location
      end
