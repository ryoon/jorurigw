class Cms::Admin::RoutesController < ApplicationController
  include System::Controller::Scaffold
  
  def initialize_scaffold
    id      = params[:parent] == '0' ? 1 : params[:parent]
    @parent = Cms::Route.new.find(id)
  end
  
  def index
    item = Cms::Route.new#.readable
  ...
  def show
    @item = Cms::Route.new.find(params[:id])
  ...
  def create
    @item = Cms::Route.new(params[:item])
    @item.parent_id = @parent.id
    _create @item
  end
