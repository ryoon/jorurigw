# -*- encoding: utf-8 -*-
class Gwworkflow::Admin::BasicsController < Gw::Controller::Admin::Base
  ...
  def show
    ...
    @item = Gwworkflow::Control.find(params[:id])
  ...
  def create
    ...
    @item = Gwworkflow::Control.new(params[:item])
  ...
  def destroy
    ...
    @title = Gwworkflow::Control.find(params[:id])
    _destroy @item
  end
  ...
  def circularadm_index
    ...
    sql = Condition.new
    sql.or {|d|
      d.and "sql", "gw_workflow_controls.state = 'public'"
      d.and "sql", "gw_workflow_controls.view_hide = 0" if hide
      d.and "sql", "gw_workflow_controls.view_hide = 1" unless hide
      d.and "sql", "gw_workflow_adms.user_code = '#{Site.user.code}'"
    }
    ...
    sql.or {|d|
      ...
      d.and "sql", "gw_workflow_controls.view_hide = 0" if hide
      d.and "sql", "gw_workflow_controls.view_hide = 1" unless hide
      d.and "sql", "gw_workflow_adms.user_id = 0"
      d.and "sql", "gw_workflow_adms.group_code = '#{Site.user_group.code}'"
    }
    join = "INNER JOIN gw_workflow_adms ON gw_workflow_controls.id = gwworkflow_adms.title_id"
    item = gwworkflow::Control.new
    item.page   params[:page], params[:limit]
    @items = item.find(:all, :joins=>join, :conditions=>sql.where,:order => 'sort_no, updated_at DESC', :group => 'gw_workflow_controls.id')
    _index @items
  end
  ...
  def destroy_categories
    item = gwworkflow_db_alias(Gwworkflow::Category)
    item.destroy_all(sql_where)
    Gwworkflow::Category.remove_connection
  end

  def destroy_docs
    item = gwworkflow_db_alias(Gwworkflow::Doc)
    item.destroy_all(sql_where)
    Gwworkflow::Doc.remove_connection
  end

  def destroy_comments
    item = gwworkflow_db_alias(Gwworkflow::Comment)
    item.destroy_all(sql_where)
    Gwworkflow::Comment.remove_connection
  end

  def destroy_image_files
    item = gwworkflow_db_alias(Gwworkflow::Image)
    item.destroy_all(sql_where)
    Gwworkflow::Image.remove_connection
  end

  def destroy_atacched_files
    item = gwworkflow_db_alias(Gwworkflow::File)
    item.destroy_all(sql_where)
    Gwworkflow::File.remove_connection
  end

  def destroy_files
    item = gwworkflow_db_alias(Gwworkflow::DbFile)
    item.destroy_all(sql_where)
    Gwworkflow::DbFile.remove_connection
  end
