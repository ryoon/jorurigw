class System::Public::ApiController < ActionController::Base
...
protected
  ...
  def api_checker
    ...
    user = JoruriContext[:current_user]
  ...
  def air_login(account, token)
    ...
    dump ['air_sso_login_token',Time.now.strftime('%Y-%m-%d %H:%M:%S'),request.parameters]
    admin_uri = "/"
    admin_uri += "?_jgw_session=#{session.session_id}" unless session.session_id.blank?
