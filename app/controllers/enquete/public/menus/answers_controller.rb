...
#フォーム情報
################################################################################
class Enquete::Public::Menus::AnswersController < ApplicationController
  ...
  def initialize_scaffold
    ...
    @field_lists = ''
    @field_lists = JsonParser.new.parse(@title.form_body) unless @title.form_body.size == 0 unless @title.form_body.blank?
  ...
  def new
    ...
    unless item.blank?
      return redirect_to item.edit_path if item.state == 'public'
  ...
  def user_check(is_show=nil)
    ...
    return redirect_to @item.show_path if @item.base.state == 'closed' unless is_show
  end
  ...
  def apart_groups
    ...
    for fld in items
      field_name_array =[]
      groups = []
      groups = JsonParser.new.parse(fld.group_body) unless fld.group_body.blank?
      field = Hash::new
