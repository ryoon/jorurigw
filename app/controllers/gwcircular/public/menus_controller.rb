class Gwcircular::Public::MenusController < ApplicationController
  ...
  def update
    ...
    if @item.state == 'draft'
      location = "#{@item.item_home_path}#{s_cond}"
    else
      location = "#{@item.item_home_path}#{@item.id}/circular_publish#{s_cond}"
    end
