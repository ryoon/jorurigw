...
#
################################################################################
class Questionnaire::Public::Menus::ResultsController < ApplicationController
  ...
  def initialize_scaffold
    ...
    @field_lists = JsonParser.new.parse(@title.form_body)
  ...
  def field_to_result_records(field_count)
    for i in 1..field_count
      ...
      if fld['question_type'].index('text').blank?
        ...
        options = JsonParser.new.parse(item.option_body) unless item.option_body.blank? unless item.blank?
      end
