class Digitallibrary::Public::CabinetsController < ApplicationController
  ...
  def bbsadm_index
    ...
    sql = Condition.new
    sql.or {|d|
      d.and "sql", "digitallibrary_controls.state = 'public'"
      d.and "sql", "digitallibrary_controls.view_hide = 0" if hide
      d.and "sql", "digitallibrary_controls.view_hide = 1" unless hide
      d.and "sql", "digitallibrary_adms.user_code = '#{Site.user.code}'"
    }
    ...
    sql.or {|d|
      ...
      d.and "sql", "digitallibrary_controls.view_hide = 0" if hide
      d.and "sql", "digitallibrary_controls.view_hide = 1" unless hide
      d.and "sql", "digitallibrary_adms.user_id = 0"
      d.and "sql", "digitallibrary_adms.group_code = '#{Site.user_group.code}'"
    }
    join = "INNER JOIN digitallibrary_adms ON digitallibrary_controls.id = digitallibrary_adms.title_id"

    item = Digitallibrary::Control.new
    item.page   params[:page], params[:limit]
    @items = item.find(:all, :joins=>join, :conditions=>sql.where,:order => 'id DESC', :group => 'digitallibrary_controls.id')
    _index @items
  end
