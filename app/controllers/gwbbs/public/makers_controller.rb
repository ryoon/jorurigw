class Gwbbs::Public::MakersController < ApplicationController
  ...
  def bbsadm_index
    ...
    sql.or {|d|
      ...
      d.and "sql", "gwbbs_adms.user_code = '#{Site.user.code}'"
    }
    ...
    sql.or {|d|
      ...
      d.and "sql", "gwbbs_adms.group_code = '#{Site.user_group.code}'"
    }

    parent_group_code = ''
    parent_group_code = Site.user_group.parent.code unless Site.user_group.parent.blank?
    unless parent_group_code.blank?
      sql.or {|d|
        d.and "sql", "gwbbs_controls.state = 'public'"
        d.and "sql", "gwbbs_controls.view_hide = 0" if hide
        d.and "sql", "gwbbs_controls.view_hide = 1" unless hide
        d.and "sql", "gwbbs_adms.user_id = 0"
        d.and "sql", "gwbbs_adms.group_code = '#{parent_group_code}'"
      }
    end
    join = "INNER JOIN gwbbs_adms ON gwbbs_controls.id = gwbbs_adms.title_id"
    item = Gwbbs::Control.new
    item.page   params[:page], params[:limit]
    @items = item.find(:all, :joins=>join, :conditions=>sql.where,:order => 'sort_no, updated_at DESC', :group => 'gwbbs_controls.id')
    _index @items
  end
