class Gwbbs::Public::DocsController < ApplicationController
  ...
  def initialize_scaffold
    ...
    return redirect_to("#{Site.current_node.public_uri}?title_id=#{params[:title_id]}&state=#{params[:state]}") if params[:reset]
  ...
  def recognize_index(no_paginate=nil)
    ...
    sql.or {|d|
      ...
      d.and "sql", "gwbbs_docs.section_code = '#{Site.user_group.code}'" unless @is_admin
    }
    sql.or {|d|
      d.and "sql", "gwbbs_docs.state = 'recognize'"
      d.and "sql", "gwbbs_recognizers.code = '#{Site.user.code}'"
    }
