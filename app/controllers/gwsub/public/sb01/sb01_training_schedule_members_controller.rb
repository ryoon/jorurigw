class Gwsub::Public::Sb01::Sb01TrainingScheduleMembersController < ApplicationController
...
#    item.creator
  ...
  def new
    ...
    @item.training_group_id     = Site.user_group.group_id
    @item.entry_user_id         = Site.user.id
    @item.entry_group_id        = Site.user_group.group_id
  end
  def create
    init_params
    @item1 = Gwsub::Sb01Training.find(@t_id)
    ...
    options = {
      ...
      :after_process=>Proc.new{
        ...
        #  :st_at            =>  @ts.schedule.st_at,
        #  :ed_at            =>  @ts.schedule.ed_at
        #  })
        skd = Gw::Schedule.new({
