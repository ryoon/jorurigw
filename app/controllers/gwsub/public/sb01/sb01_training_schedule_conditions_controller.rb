require "pp"
class Gwsub::Public::Sb01::Sb01TrainingScheduleConditionsController < ApplicationController
...
#    item.creator
    ...
    @items = item.find(:all)
#    @item = Gwsub::Sb01Training.find(@items[0].training_id) unless @items.blank?
#    @item = Gwsub::Sb01Training.find(params[:t_id]) if @items.blank?
    _index @items
  end
  def show
    init_params
...
#    location = "#{Site.current_node.public_uri}?t_id=#{params[:item]['training_id']}"
  ...
  def edit
    init_params
    @item = Gwsub::Sb01TrainingScheduleCondition.find(params[:id])
    #@item.repeat_monthly = Gwsub::Sb01TrainingScheduleCondition.monthly_flg(@item.repeat_monthly)
    #@item.repeat_weekday = Gwsub::Sb01TrainingScheduleCondition.weekday_flg(@item.repeat_weekday)
    @item1 = Gwsub::Sb01Training.find(@item.training_id)
  end
  def update
    init_params
    ...
    location = "#{Site.current_node.public_uri}#{@item.id}?t_id=#{@item.training_id}"
...
#    location = Gw.chop_with("#{Site.current_node.public_uri}",'/')
  ...
  def destroy_schedule
    ...
    unless t_skd.blank?
      ...
      #Gwsub::Sb01TrainingSchedule.destroy_all("condition_id = #{@item.id}")
    end
  end
...
#    ActiveRecord::Base.transaction do
      #count=0
      repeats.each do |s|
        # 予約可能日のチェック　管財との連携は外したのでチェックしない
#pp 'Gwsub::Public::Sb01::Sb01TrainingScheduleConditionsController',Gwsub::Sb01TrainingScheduleProp.prop_date_reservable(s[1])
        #next if Gwsub::Sb01TrainingSchedule.date_reservable(s[1])==false
        # schedule
        #count = count+1
        skd = Gw::Schedule.new({
