class Gwsub::Public::Sb01::Sb01TrainingSchedulesController < ApplicationController
...
#    item.creator
  ...
  def update
    ...
    @item.from_end   = ed_at
    location = "#{Site.current_node.public_uri}#{@item.id}?t_id=#{@item.training_id}&t_menu=plans"
...
#    @sort_keys = nz(params[:sort_keys], 'fyear_markjp DESC , categories ASC , bbs_url ASC' )
  ...
  def closed
    ...
    location = "#{Site.current_node.public_uri}#{@item.id}?t_id=#{@t_id}&t_menu=#{@top_menu}"
    options = {
      :success_redirect_uri=>location,
    }
    _update(@item,options)
...
#    m_join   << " left join dev_jgw_core.system_users on dev_jgw_core.system_users.id=dev_jgw_gw_pref.gwsub_sb01_training_schedule_members.training_user_id "
#    members = Gwsub::Sb01TrainingScheduleMember.find(:all,:conditions=>"training_schedule_id=#{params[:p_id]}",:order=>m_order,:joins=>m_join)
    ...
    unless (members.blank? && training.blank? && skd.blank?)
      members.each do |m|
        rets += %Q("#{m.user_rel1.code}","#{m.user_rel1.offitial_position}","#{m.user_rel1.name}","#{m.group_rel1.name}","#{m.training_user_tel}")
      ...
      send_download "#{filename}", NKF::nkf(type_sjis,rets)
    else
      location = Gw.chop_with("#{Site.current_node.public_uri}",'/') + "/?t_id=#{params[:t_id]}&t_menu=entries"
      redirect_to location
    end
  end
end
