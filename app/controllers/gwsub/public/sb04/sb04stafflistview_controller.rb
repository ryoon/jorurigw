class Gwsub::Public::Sb04::Sb04stafflistviewController < ApplicationController
...
#    error_auth unless (@content = Cms::Content.find(params[:content]))
  ...
  def create
    ...
    location = "#{Site.current_node.public_uri}""#{params[:item]['user_id']}"
    options = {
  ...
  def csvput
    ...
    when 'put'
      ...
      if par_item[:select]=='all'
        select = "section_code,section_name,assignedjobs_code,assignedjobs_name,divide_duties_order,assignedjobs_tel,staff_no,divide_duties,official_title_name,categories_name,name,name_print,kana,extension,remarks"
      else
        cols = par_item[:chks].to_a.sort.collect{|k,v| v}
        sel_cols = cols.join(',')
        select = "section_code,section_name,assignedjobs_name,assignedjobs_tel,staff_no,official_title_name,categories_name,name"
      ...
      if staffs.blank?
        ...
        select_jp << "\n"
        select_jp << file
        send_download "#{filename}", NKF::nkf(nkf_options,select_jp)
      end
    else
    end
  end
  def csvview
    ...
    when 'put'
      ...
      if staffs.blank?
        ...
        group_code = 'all' if par_item[:grped_id].to_i == 0
        group_code = staffs[0].section_name unless par_item[:grped_id].to_i == 0
        
        if par_item[:form] == "csv"
          filename = "#{fyear_markjp}_Syokuinroku_#{group_code}_#{par_item[:nkf]}"
          file = Gwsub::Script::Tool.stafflistview_to_csv(staffs, :cols=>select,:header=>false,:quotes=>true)
          select_jp << "\n"
          select_jp << file
          send_download "#{filename}.csv", NKF::nkf(nkf_options,select_jp)
        end
      end
    else
    end
