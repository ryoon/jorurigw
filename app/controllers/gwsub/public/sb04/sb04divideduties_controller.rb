class Gwsub::Public::Sb04::Sb04dividedutiesController < ApplicationController
...
#pp params
    ...
    @items = item.find(:all)
    # 登録後は、選択したユーザーの詳細表示。
    location = "#{Site.current_node.public_uri}""#{params[:item]['user_id']}#{@param}"
    options = {
  ...
  def assigned_job_edit
    ...
    if @item.assignedjobs_id.to_i==0
      ...
      @assigned_job = Gwsub::Sb04assignedjob.find(@item.assignedjobs_id)
    end
  end
