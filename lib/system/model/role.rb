module System::Model::Role
  ...
  def self.get(class_id, uid, table_name, priv_name)
    nz(self.get_raw(class_id, uid, table_name, priv_name),0) == 1 ? true : nil
  end

  def self.get_raw(class_id, uid, table_name, priv_name)
    case class_id
    when 1
      gid_cond = " user_id=#{uid} and end_at is null"
      gid_wrk = System::UsersGroup.find(:first , :conditions=>gid_cond ,:order=>"job_order")
      gid = nz(gid_wrk.group_id,'')
    when 2
      gid = uid
    end
    uid = uid.to_i
    ...
    roles.each do |role|
      case nz(role.class_id,0)
      when 0
        return nz(role.priv,0)
      when 1
        next if class_id == 2
        return nz(role.priv,0) if nz(role.uid,'').to_s == uid.to_s
      when 2
        return nz(role.priv,0) if !gid_parents.index(nz(role.uid.to_i,'')).nil?
      end
    end
  ...
  def self.get_roles(uid = Site.user.id)
